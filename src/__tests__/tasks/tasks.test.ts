...
describe("toShellCommand()", () => {
  ...
  test("with a name", () => {
    ...
    // THEN
    expect(t.toShellCommand()).toMatchSnapshot();
    expect(shell(t)).toStrictEqual(["STEP I", "step 1"]);
  });
  ...
  test("with a condition", () => {
    ...
    expect(shell(t, { RUNME: "1" })).toStrictEqual(["STEP I", "hello, world"]);
  });
  ...
  test("with environment", () => {
    ...
    expect(shell(t1)).toStrictEqual(["hello, world !"]);
  });
});

function shell(t: Task, env: { [k: string]: string } = {}) {
  const result = spawnSync(t.toShellCommand(), {
    shell: true,
    env: { ...process.env, ...env },
  });
  if (result.status !== 0) {
    throw new Error(
      `non-zero exit code ${result.status}: ${result.stderr.toString("utf-8")}`
    );
  }
